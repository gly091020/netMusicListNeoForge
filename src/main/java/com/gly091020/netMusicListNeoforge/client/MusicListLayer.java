...
public class MusicListLayer implements LayeredDraw.Layer{
    ...
    public void render(@NotNull GuiGraphics guiGraphics, @NotNull DeltaTracker deltaTracker) {
        ...
        if(index - 5 < 0){
            for(int l = 0; l < 10; l++){
                if(l >= songList.size()){continue;}
                guiGraphics.drawString(font, songList.get(l).songName, width - max_width - 5, (int) (height * 0.3 + j * (font.lineHeight + 2)), 0xFFFFFF);
                if(l == index){
                    guiGraphics.fill(width - max_width - 7, (int) (height * 0.3 + j * (font.lineHeight + 2)),
                            width - 7, (int) (height * 0.3 + (j + 1) * (font.lineHeight + 2)), 0x80FFFFFF);
                }
                j++;
            }
        } else if (index + 5 >= songList.size()) {
